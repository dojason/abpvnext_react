...
const EditForm: React.FC<EditFormProps> = props => {
  ...
  const storeFormData = (inputName:string)=>
  {
    return (e:any)=>
    {
  ...
  //console.log({...editClaimTypeItem,valueType:valueFormat});
  ...
  return (
    ...
    <Form form={form} layout="vertical">
      <Form.Item rules={[{
        ...
        {/*<Input onChange={storeFormData('name')} />*/}
        <Input/>
      </Form.Item>
      <Form.Item rules={[{
        required: true,
      ...
      <Form.Item label="数据类型" name="valueType">
        <Select>
          {
            valueType.map(item => <Option value={item}  ken={item}>{item}</Option>)
          }
        </Select>
      </Form.Item>
