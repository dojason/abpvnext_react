...
import { IdentityResourceWithDetailsDto } from '@/pages/admin/identity-server/identity-resources/data';
...
interface IIdentityResourceProps
{
  dispatch:Dispatch;
...
const IdentityResources :React.FC<IIdentityResourceProps> = (props)=>
{
  const { dispatch, identityResource } = props;
  ...
  useEffect(() => {
    dispatch({
      type:'identityResource/getIdentityResource',
      payload:null,
    });
  },[]);
...
const mapStateToProps =({ identityResource }:ConnectState) =>{
  return {
    identityResource:identityResource.identityResource
  }};
export default connect(mapStateToProps)(IdentityResources);;
